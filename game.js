...
function roomJackTarget(room, uid, targetUid, targetSlot){
  ...
  // Store who swapped with whom and which slot — shown as a notification to victim
  room.modal = {
    type:'swapDone',
    withName: opp.name,
    swapperName: p.name,
    victimUid: targetUid,
    victimSlot: targetSlot  // slot index in victim's hand (the card that moved away)
  };
  return {};
}
...
function updateCardSize(){
  ...
  // Fixed chrome: header~50px, actions~50px, midrow~52px, name rows~30px, gaps~20px
  const fixedH = 50 + 50 + 52 + 30 + 20;
  const remaining = vh - fixedH;

  // Give opponents 42%, player 58% of remaining space
  const oppAreaH = remaining * 0.44;
  const playerAreaH = remaining * 0.50;

  // Cards per row in opponent blocks (lay them out in rows of cards side by side)
  const cardsPerOppRow = Math.max(4, Math.floor(availW / 2 / 36)); // ~half width each block
  const oppRowsPerBlock = Math.ceil(maxOppCards / cardsPerOppRow);
  const cardHFromOpp = Math.floor((oppAreaH / numOpponents - 8) / (oppRowsPerBlock + 0.3));
  const cardWFromOpp = Math.floor(cardHFromOpp * (2/3));

  // Player hand: fit all cards in one row width-wise
  const cardWFromMyHand = Math.floor((availW - (myCards-1)*6) / myCards);
  // Also constrain by height of player area
  const cardWFromPlayerH = Math.floor((playerAreaH - 8) * (2/3));

  // Take the minimum so nothing overflows, but keep a generous floor
  const minSize = Math.max(36, Math.floor(availW / 8)); // at least 1/8 of screen width
  const final = Math.max(minSize, Math.min(cardWFromOpp, cardWFromMyHand, cardWFromPlayerH, 88));
...
function renderModal(){
  ...
  if(m.type==='swapDone'){
    const isVictim = m.victimUid === myUid;
    const isSwapper = ROOM.currentUid === myUid;
    const title = t('swapDoneTitle');
    const body = isVictim
      ? t('swapVictimBody', m.swapperName, m.victimSlot+1)
      : t('swapDoneBody', m.withName);
    return modalWrap(`<h3>${title}</h3>
      <p style="font-size:14px;color:${isVictim?'var(--crimson)':'var(--brass-soft)'}">${body}</p>
