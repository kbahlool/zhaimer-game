...
/* ============================= ADAPTIVE CARD SIZE =============================
   ...
   everything fits on one screen without scrolling. Called after every render. */
function updateCardSize(){
  ...
  // Max cards any opponent holds (for row calculation)
  const maxOppCards = others.reduce((m,u)=>Math.max(m,(ROOM.players[u]?.hand||[]).length),0) || 4;

  // Layout budget: header~56px, actions~46px, midrow~56px, gaps~24px, player-row-top~36px
  const fixedH = 56 + 46 + 56 + 24 + 36;
  // Remaining height split: 40% for opponents, 60% for player hand
  const remaining = vh - fixedH;
  const oppAreaH = remaining * 0.38;
  const playerAreaH = remaining * 0.52;

  // Opponent rows: stack vertically per opponent, 2 rows of cards each
  const oppRowsPerBlock = Math.ceil(maxOppCards / Math.floor(availW / 40));
  const cardHFromOpp = Math.floor((oppAreaH / numOpponents) / (oppRowsPerBlock + 0.4));
  const cardWFromOpp = Math.floor(cardHFromOpp * (2/3));

  // Player hand: all in one row ideally
  const cardWFromMyHand = Math.floor((availW - (myCards-1)*5) / myCards);
  const cardHFromMyHand = Math.floor(cardWFromMyHand * 1.5);
  const cardWFromPlayerH = Math.floor(playerAreaH / 1.5); // height-constrained

  const final = Math.max(24, Math.min(cardWFromOpp, cardWFromMyHand, cardWFromPlayerH, 72));
...
function renderGameOver(){
  ...
  <div class="setup-card" style="text-align:center">
    ...
    <button class="primary-btn" style="margin-top:18px;width:100%" data-action="actLeaveToMenu">${t('playAgainBtn')}</button>
    ${donateBtn}
  </div>`;
}
