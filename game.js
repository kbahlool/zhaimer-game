...
function updateCardSize(){
  ...
  // More opponents = more grid columns, so rows (and total page height) stay
  // reasonable instead of always stacking in a fixed 2-column layout.
  const oppCols = numOpponents >= 5 ? 3 : 2;
  document.documentElement.style.setProperty('--opp-cols', oppCols);
  document.body.classList.toggle('many-players', numOpponents >= 4);

  // Chrome heights (smaller in compact/many-player mode)
  const headerH = compact ? 40 : 64;
  const actionsH = 46;
  const midrowH = compact ? 44 : 64;
  const playerNameH = compact ? 26 : 34;
  const gaps = 14;
  ...
  const oppBlockNameH = compact ? 20 : 26;
  ...
  const minSize = numOpponents >= 5 ? 32 : numOpponents >= 4 ? 40 : Math.max(40, Math.floor(availW / 10));
...
function renderModal(){
  ...
  if(m.type==='swapDone'){
    // Note: only the swapper (current turn player) ever reaches this —
    // the victim always sees just the generic "waiting for..." banner
    // above, with no mention of the swap at all. That's intentional:
    // the Jack power stays completely silent for whoever was targeted.
