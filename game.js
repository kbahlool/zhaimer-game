...
const I18N = {
  ...
  ar:{
    ...
    rulesBody:`
      ...
      <ul>
        <li>الآس (A) = 1 نقطة</li>
        <li>الأوراق من 2 إلى 9 = قيمتها المكتوبة عليها</li>
        <li><b>10 الأحمر (كبة / ديناري) = 0 نقطة</b> — قيمة استثنائية!</li>
        <li><b>10 الأسود (بستوني / سباتي) = 10 نقاط</b></li>
        <li>أوراق <b>J</b> و<b>Q</b> و<b>K</b> = 10 نقاط لكلٍّ منها</li>
      </ul>
      ...
      <h3>👑 القوى الخاصة — K و Q و J</h3>
      <p>إذا سحبت إحدى هذه الأوراق الخاصة من المجموعة واخترت رميها مباشرةً (لا تبديلها بيدك)، فبإمكانك تفعيل قوتها الخاصة. أما إذا بدَّلتَ بها إحدى أوراقك الموجودة في يدك، فلا تُفعَّل أي قوة.</p>
      <ul>
        <li>
          <b>K (الملك)</b> — اسحب ورقتين إضافيتين، ثم اختر بين ثلاثة خيارات:
          <ul style="margin-top:4px">
            <li>احتفظ بإحداهما فقط: بدِّلها بإحدى أوراق يدك وارمِ الأخرى</li>
            <li>احتفظ بكلتيهما: بدِّل كلتيهما بورقتين من يدك</li>
            <li>ارمِ كلتيهما دون الاحتفاظ بأي منهما</li>
          </ul>
        </li>
        <li><b>J (الولد)</b> — مبادلة عمياء: تبادل إحدى أوراقك مع ورقة عشوائية لدى أحد المنافسين، دون أن يرى أيٌّ منكما الورقتين المتبادَلتين.</li>
        <li><b>Q (البنت)</b> — اطَّلع سرًّا على إحدى أوراقك المخفية.</li>
      </ul>
...
function updateCardSize(){
  ...
  // Chrome heights
  const headerH = compact ? 44 : 64;
  const actionsH = 50;
  const midrowH = compact ? 50 : 64;
  const playerNameH = 34;
  const gaps = 16;
  const fixedH = headerH + actionsH + midrowH + playerNameH + gaps;
  const remaining = Math.max(100, vh - fixedH);

  // Opponents grid: 2 columns, rows = ceil(numOpps/2)
  const oppRows = Math.ceil(numOpponents / 2);
  const oppAreaH = remaining * 0.44;
  const oppBlockH = (oppAreaH / oppRows) - 6;
  const oppBlockNameH = 26;
  const availOppCardH = oppBlockH - oppBlockNameH;
  const halfW = (availW / 2) - 8;
  const cardsPerOppRow = Math.max(2, Math.ceil(maxOppCards / 2));
  const cardWFromOppW = Math.floor((halfW - (cardsPerOppRow-1)*4) / cardsPerOppRow);
  ...
  const minSize = Math.max(40, Math.floor(availW / 10));
