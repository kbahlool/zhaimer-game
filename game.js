...
function offerFinishCheck(room, uid){
  ...
  room.finishCheckUntil = Date.now() + 5000;
}
...
const I18N = {
  ...
  ar:{
    ...
    rulesBody:`
      ...
      <p>بعد ما تخلص دورك العادي (تسحب، تبدّل/ترمي، تستخدم قوة، أو تحاول تحرق)، بتاخذ <b>5 ثواني</b> فرصة تعلن فيها <b>الانتهاء</b> إذا تعتقد إن مجموعك المخفي هو الأقل. لو ما رديت بالوقت (أو قلت لا)، اللعبة تكمل عادي للاعب اللي بعدك. إعلان الانتهاء ينهي الجولة — كل اللاعبين الباقين ياخذون دور أخير وحد، وبعدها تنكشف كل الأيدي.</p>
      <ul>
...
function renderModal(){
  ...
  const m = ROOM.modal;
  if(m.type==='finishCheck'){
    const secs = finishCheckSecondsLeft();
    return `<div class="hint-banner">
      <button class="primary-btn" style="width:100%;border-color:var(--teal);background:linear-gradient(180deg,var(--teal),#2fa393)" data-action="actFinishCheckAnswer" data-val="yes">${t('declareFinishedBtn')} (${secs===null?5:secs}s)</button>
    </div>`;
  }
...
function render(){
  ...
  let html = '';
  if(!(ROOM && ROOM.phase==='playing' && ROOM.currentUid===myUid && !ROOM.drawnCard && !ROOM.modal)){ burnDeclared = false; declareConfirmUntil = null; }
