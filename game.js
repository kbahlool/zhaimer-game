...
function dealNewRound(room){
  ...
  const startUid = room.turnOrder[startIdx];
...
function roomAnswerAbility(room, uid, useIt){
  ...
  if(card.rank==='K'){
    ...
    if(room.deck.length<2){ advanceTurn(room); return {}; }
...
/* ---- Burn: on your own turn, instead of drawing, you may try to throw down a
   card matching whatever's currently on top of the discard pile. Guess right
   and it's removed along with the pile's top card (hand shrinks by one).
   Guess wrong and you draw 2 penalty cards instead. Either way, this uses up
   your turn — play passes to the next player afterward. ---- */
function roomAttemptBurn(room, uid, slotIdx){
...
function revealAndScore(room){
  ...
  for(const s of scores){
    if(s.score===minScore){ s.roundScore = 0; roundWinnerNames.push(room.players[s.uid].name); }
  }
  for(const s of scores){
...
if(typeof module !== 'undefined'){
  module.exports = {
    ...
    roomDeclareFinished, revealAndScore, roomNextRound,
...
const DONATE_URL = "https://ko-fi.com/kmbgames";
...
const AI_NAMES = ['Mireille','Kasimir','Odalys','Ferro','Junie'];
...
async function aiTakeTurnLocal(uid){
  ...
  // Burn attempt (uses up the turn): only if AI is genuinely confident it holds a match
  if(!room.roundEnding && room.discard.length>0){
    const targetRank = room.discard[room.discard.length-1].rank;
    const confThreshold = DIFFICULTY==='hard'?0.5 : DIFFICULTY==='medium'?0.75 : 2;
    let bestSlot=-1, bestConf=-1;
    for(let i=0;i<p.hand.length;i++){
      const m = p.mem[i];
      if(m && m.known && m.rank===targetRank && m.conf>=confThreshold && m.conf>bestConf){ bestSlot=i; bestConf=m.conf; }
    }
    if(bestSlot>=0){
      roomAttemptBurn(room, uid, bestSlot);
      render();
      return;
    }
  }

  if(!room.roundEnding && (room.turnCount||0)>=6){
    ...
    if(est<=threshold && avgConf>0.45 && Math.random()<willingness){
      roomDeclareFinished(room, uid);
      render();
      return;
    }
  }
...
const I18N = {
  en:{
    ...
    termsBody:`
      <p><i>Last updated: 2026. This is a general template — have a lawyer review it before relying on it for a real business.</i></p>
      <h3>Using this game</h3>
      <p>This game is provided for entertainment purposes, free to play, "as is," with no guarantee of uptime, accuracy, or fitness for any particular purpose.</p>
      <h3>Acceptable use</h3>
      <p>Don't use this game to harass others, attempt to disrupt the service, or exploit bugs to cheat in ways that ruin other players' experience.</p>
      <h3>No warranty</h3>
      <p>We do our best to keep the game working, but we're not liable for any loss or damage arising from your use of it, to the fullest extent permitted by law.</p>
      <h3>Changes</h3>
      <p>These terms and the game itself may change over time. Continued use after changes means you accept the updated terms.</p>
      <h3>Contact</h3>
      <p><i>labgameskmb@gmail.com</i></p>
    `,
    rulesBody:`
      <h3>🎯 The Goal</h3>
      ...
      <p>Instead of drawing on your turn, you may declare <b>Finished</b> if you believe your hidden total is the lowest. This ends the round — everyone else gets exactly one more turn, then all hands are revealed.</p>
      <ul>
    ...
    burnBtn:'Burn', pickBurnCard:'Click the card you think matches the top of the discard pile.',
  ...
  ar:{
    title:'زايمر', subtitle:'أونلاين — العب مع أصحابك', soundBtn:'تشغيل/كتم الصوت',
    ...
    howToPlayBtn:'كيف تلعب', rulesTitle:'كيف تلعب زايمر',
    ...
    termsBody:`
      <p><i>آخر تحديث: 2026. هذا نص عام مبدئي — راجعه مع محامي قبل ما تعتمد عليه لمشروع تجاري حقيقي.</i></p>
      <h3>استخدام اللعبة</h3>
      <p>هذي اللعبة مقدّمة لأغراض ترفيهية، مجانية، "كما هي"، بدون ضمان لاستمرارية الخدمة أو دقتها أو ملاءمتها لأي غرض معين.</p>
      <h3>الاستخدام المقبول</h3>
      <p>لا تستخدم اللعبة لمضايقة الآخرين، أو محاولة تعطيل الخدمة، أو استغلال أخطاء برمجية بطريقة تفسد تجربة اللاعبين الآخرين.</p>
      <h3>بدون ضمان</h3>
      <p>نبذل جهدنا نخلي اللعبة تشتغل صح، بس ما نتحمل مسؤولية أي خسارة أو ضرر ناتج عن استخدامك لها، إلى أقصى حد يسمح به القانون.</p>
      <h3>التغييرات</h3>
      <p>هذي الشروط واللعبة نفسها ممكن تتغير مع الوقت. استمرارك باستخدام اللعبة بعد أي تغيير يعني موافقتك على الشروط الجديدة.</p>
      <h3>تواصل</h3>
      <p><i>labgameskmb@gmail.com</i></p>
    `,
    rulesBody:`
      <h3>🎯 الهدف</h3>
      <p>بكل جولة، هدفك تحتفظ بيد أوراق مخفية <b>بأقل مجموع قيمة ممكن</b> — من غير ما تعرف بالضبط شو فيها طول الوقت. الذاكرة والمخاطرة وشوي من الجرأة هي اللي بتحدد الفايز بكل جولة. اللعبة تستمر جولة بعد جولة لين ما يبقى إلا لاعب واحد تحت 100 نقطة.</p>

      <h3>🃏 التوزيع</h3>
      <p>كل لاعب ياخذ <b>4 أوراق مقلوبة</b>. بأول الجولة، تقدر تنظر لـ<b>ورقتين من الأربعة</b> لمدة 4 ثوانٍ — وبعدها ترجع تنقلب مخفية. من هاللحظة، ذاكرتك هي كل اللي عندك؛ ما فيه تأكيد مجاني بعدها.</p>

      <h3>🔢 قيم الأوراق</h3>
      <ul>
        <li>الآص (A) = 1</li>
        <li>2–9 = نفس الرقم المكتوب</li>
        <li><b>10 الكبة / الديناري (الأحمر) = 0</b> — صفقة سرية!</li>
        <li><b>10 البستوني / السباتي (الأسود) = 10</b></li>
        <li>الولد، البنت، الملك = 10 (كأوراق تحسب بالنتيجة)</li>
      </ul>

      <h3>🔁 دورك</h3>
      <p>بدورك: <b>اسحب</b> ورقة، إما من المجموعة المقلوبة (مجهولة) أو من كومة الرمي (قيمتها معروفة للجميع). وبعدها اختر:</p>
      <ul>
        <li><b>بدّلها بورقة عندك</b> — الورقة اللي تستبدلها تنرمى مكشوفة</li>
        <li><b>ارمِها مباشرة</b> — مسموح بس إذا سحبتها من المجموعة المقلوبة</li>
      </ul>

      <h3>👑 القوى الخاصة (الملك، الولد، البنت)</h3>
      <p>لو سحبت ملك أو ولد أو بنت من المجموعة واخترت ترميها مباشرة (بدل ما تبدّلها بيدك)، تقدر تفعّل قوتها الخاصة بدل ما ترميها عادي. تبديل ورقة خاصة برا يدك <b>ما</b> يفعّل قوتها — بس السحب الجديد من المجموعة يقدر يسوي هذا.</p>
      <ul>
        <li><b>الملك</b> — اسحب ورقتين جديدتين. احتفظ بوحدة (بدّلها بيدك وارمِ الباقي)، أو احتفظ بالاثنتين (بدّل الاثنتين بيدك، وارمِ الورقتين اللي استبدلتهم)، أو ارمِ الاثنتين.</li>
        <li><b>الولد</b> — تبديل أعمى: بدّل وحدة من أوراقك مع ورقة أحد الخصوم، بدون ما يشوف أي واحد منكم الورقتين.</li>
        <li><b>البنت</b> — انظر سرًا لإحدى أوراقك الخاصة.</li>
      </ul>

      <h3>🔥 الحرق</h3>
      <p>بدورك أنت، بدل ما تسحب، تقدر تضغط <b>🔥 حرق</b> إذا تعتقد إن عندك ورقة تطابق أعلى ورقة موجودة حاليًا بكومة الرمي. اختر الورقة اللي تعتقد إنها متطابقة — لو خمّنت صح، تُشال الورقتين من اللعب وينقص عدد أوراقك بورقة. لو خمّنت غلط، تسحب <b>ورقتين كعقوبة</b>. بكل الحالتين، هذا يستهلك دورك بالكامل — الدور ينتقل للاعب اللي بعدك مباشرة.</p>

      <h3>🏁 إعلان "الانتهاء"</h3>
      <p>بدل ما تسحب بدورك، تقدر تعلن <b>الانتهاء</b> إذا تعتقد إن مجموعك المخفي هو الأقل. هذا ينهي الجولة — كل اللاعبين الباقين ياخذون دور أخير وحد، وبعدها تنكشف كل الأيدي.</p>
      <ul>
        <li>لو كنت صح (أقل مجموع بشكل قاطع) — ممتاز، مخاطرتك نجحت.</li>
        <li>لو كنت غلط — نقاطك بهذي الجولة <b>تتضاعف</b> كعقوبة.</li>
      </ul>

      <h3>🏆 مكافأة الفوز بالجولة</h3>
      <p>أي لاعب طلع عنده فعليًا أقل مجموع بهذي الجولة — سواء أعلن الانتهاء أو لا — ياخذ <b>0 نقطة</b> بدل مجموعه الحقيقي.</p>

      <h3>⏱ مؤقت الدور</h3>
      <p>كل لاعب عنده <b>60 ثانية</b> يتصرف بدوره. لو خلص الوقت، دوره يكتمل تلقائيًا بإجراء افتراضي آمن عشان اللعبة تستمر.</p>

      <h3>☠️ الإقصاء</h3>
      <p>أول ما يوصل مجموع أي لاعب لـ <b>100 نقطة</b>، يُقصى من اللعبة. اللعبة تستمر مع الباقين لين ما يبقى لاعب واحد بس — وهو الفايز.</p>
    `,
    ...
    burnBtn:'حرق', pickBurnCard:'اضغط على الورقة اللي تعتقد إنها مطابقة لأعلى ورقة بكومة الرمي.',
...
function declareBurnAttempt(){ burnDeclared = true; render(); }
function cancelBurnAttempt(){ burnDeclared = false; render(); }
...
function setNumAIVal(n){ NUM_AI=n; render(); }
function setDifficultyVal(d){ DIFFICULTY=d; render(); }
function submitAISetup(){ startLocalGame(NUM_AI, DIFFICULTY); }
...
/* ============================= IN-GAME ACTIONS ============================= */
function actStartGame(){ updateRoom(room=>roomStartGame(room)); }
...
function actAttemptBurn(slot){
  burnDeclared = false;
  const handLenBefore = ROOM.players[myUid].hand.length;
  updateRoom(room=>roomAttemptBurn(room, myUid, slot));
  setTimeout(()=>{
...
function actDeclareFinished(){ updateRoom(room=>roomDeclareFinished(room, myUid)); }
  ...
  updateRoom(room=>{
    ...
    if(room.modal){
      ...
      if(type==='jackTarget'){
        ...
        if(!opts.length){ room.modal=null; advanceTurn(room); return {}; }
    ...
    if(!room.drawnCard){
      ...
      return roomDiscardDrawn(room, forUid);
    } else {
...
/* ============================= RENDER: SCREENS ============================= */
function screenHeader(subtitleOverride, showQuit){
  return `<div class="header">
    <div>
      <div class="title">${t('title')}</div>
      <div class="tagline">${subtitleOverride || t('subtitle')}</div>
    </div>
    <div class="choice-group" style="align-self:flex-start">
      ...
      <button class="choice-btn ${LANG==='en'?'active':''}" data-action="setLang" data-val="en">English</button>
      <button class="choice-btn ${LANG==='ar'?'active':''}" data-action="setLang" data-val="ar">العربية</button>
    </div>
  </div>`;
}

function renderLandingEmblem(){
  return `<svg class="landing-emblem" viewBox="0 0 260 220" xmlns="http://www.w3.org/2000/svg">
    <defs>
      <radialGradient id="crestGlow" cx="50%" cy="45%" r="65%">
        <stop offset="0%" stop-color="#3a2f10" stop-opacity="0.55"/>
        <stop offset="100%" stop-color="#0b0f1a" stop-opacity="0"/>
      </radialGradient>
      <linearGradient id="brassGrad" x1="0%" y1="0%" x2="100%" y2="100%">
        <stop offset="0%" stop-color="#e9c65c"/>
        <stop offset="100%" stop-color="#c9a227"/>
      </linearGradient>
    </defs>
    <ellipse cx="130" cy="108" rx="128" ry="105" fill="url(#crestGlow)"/>
    <circle cx="130" cy="108" r="86" fill="none" stroke="url(#brassGrad)" stroke-width="2"/>
    <circle cx="130" cy="108" r="78" fill="none" stroke="#c9a227" stroke-width="1" opacity="0.45"/>
    <polygon points="130,32 206,108 130,184 54,108" fill="none" stroke="url(#brassGrad)" stroke-width="2.5"/>
    <polygon points="130,58 180,108 130,158 80,108" fill="none" stroke="#e9c65c" stroke-width="1.4" opacity="0.85"/>
    <circle cx="130" cy="108" r="6" fill="#e9c65c"/>
    <text x="130" y="26" text-anchor="middle" font-size="19" fill="#e9c65c" font-family="Georgia, serif">♠</text>
    <text x="130" y="200" text-anchor="middle" font-size="19" fill="#c1495a" font-family="Georgia, serif">♥</text>
    <text x="26" y="115" text-anchor="middle" font-size="19" fill="#c1495a" font-family="Georgia, serif">♦</text>
    <text x="234" y="115" text-anchor="middle" font-size="19" fill="#e9c65c" font-family="Georgia, serif">♣</text>
  </svg>`;
}
...
function renderLanding(){
  ...
  <div class="setup-card">
    <h2>${t('modeQuestion')}</h2>
    <div class="setup-row">
      <button class="primary-btn" style="width:100%" data-action="goAISetup">${t('playAIBtn')}</button>
    </div>
    <div class="setup-row">
      <button class="ghost-btn" style="width:100%;padding:13px" data-action="goCreate" ${!fbReady?'disabled':''}>${t('createRoomBtn')}</button>
    </div>
    <div class="setup-row">
      <button class="ghost-btn" style="width:100%;padding:13px" data-action="goJoin" ${!fbReady?'disabled':''}>${t('joinRoomBtn')}</button>
    </div>
    <div class="setup-row">
      <button class="ghost-btn" style="width:100%;padding:11px;border-color:var(--teal);color:var(--teal)" data-action="goRules">📖 ${t('howToPlayBtn')}</button>
    </div>
    ${fbNote}
  </div>
  <div class="ad-slot" id="adSlotLanding">
    <!-- AdSense placeholder: once approved, paste your <ins class="adsbygoogle">
         unit code here (or use adsbygoogle.js auto-ads and remove this div). -->
...
function renderPlayerBlock(uid){
  ...
  for(let i=0;i<p.hand.length;i++){
    ...
    cardsHtml += `<div style="position:relative">${renderCardBack(cls)}${targetable?`<div style="position:absolute;inset:0" data-action="actJackTarget" data-player="${uid}" data-slot="${i}"></div>`:''}</div>`;
  }
...
function renderMyHand(){
  ...
  for(let i=0;i<me.hand.length;i++){
    ...
    else if(burnAttempt){ inner = renderCardBack('clickable own-slot-target'); action = `data-action="actAttemptBurn" data-slot="${i}"`; }
    else { inner = renderCardBack(); }
    html += `<div ${action} style="position:relative"><div class="slot-label">${t('slotLabel')} ${i+1}</div>${inner}</div>`;
  }
  return html;
}

function renderMidRow(){
  ...
  return `<div class="midrow">
    ...
    <div class="pile">
      ...
      ${topDiscard? renderCardFace(topDiscard) : renderCardBack()}
...
function renderActions(){
  ...
  if(!ROOM.drawnCard){
    if(burnDeclared){
      return `<div class="actions"><div class="hint" style="color:var(--muted);font-size:12.5px;">${t('pickBurnCard')}</div>
        <button class="ghost-btn" data-action="cancelBurnAttempt">${t('backBtn')}</button></div>`;
    }
    return `<div class="actions">
      ...
      ${!ROOM.roundEnding?`<button class="ghost-btn" data-action="actDeclareFinished" style="border-color:var(--teal);color:var(--teal)">${t('declareFinishedBtn')}</button>`:''}
    </div>`;
  }
...
function renderModal(){
  ...
  if(m.type==='swapDone'){
    return modalWrap(`<h3>${t('swapDoneTitle')}</h3><p style="font-size:15px;color:var(--brass-soft)">${t('swapDoneBody', m.withName)}</p>
...
function renderReveal(){
  ...
  return `${screenHeader(`${t('round')} ${ROOM.round}`, true)}
  <div class="setup-card">
    <h2>${t('revealHeading', ROOM.round)}</h2>
    ${results.map(r=>{
      ...
      <div style="display:flex; align-items:center; justify-content:space-between; padding:10px 0; border-bottom:1px solid var(--line);">
        <div style="display:flex; gap:10px; align-items:center;">
          <b style="min-width:90px;display:inline-block">${r.name}${ROOM.finishedBy===r.uid?' 🎯':''}${isRoundWinner?' 🏆':''}</b>
          <div style="display:flex;gap:4px">${(r.hand||[]).map(c=>renderCardFace(c,'mini')).join('')}</div>
        </div>
        <div style="text-align:right">
          <div style="font-size:12px;color:var(--muted)">${t('rawLabel')} ${r.score}</div>
          <div style="font-weight:700;color:${scoreColor}">+${r.roundScore}${r.roundScore>r.score?' '+t('penaltyTag'):''}${isRoundWinner && r.score>0? ' '+t('roundWinTag'):''}</div>
        </div>
      </div>`;}).join('')}
    ${ROOM.roundWinnerName? `<p style="color:var(--teal);margin-top:14px;">${t('roundWinnerMsg', ROOM.roundWinnerName)}</p>`:''}
    ${ROOM.penaltyApplied? `<p style="color:var(--crimson);margin-top:6px;">${t('penaltyMsg', ROOM.penaltyApplied)}</p>` : (ROOM.finishedBy!==null? `<p style="color:var(--brass-soft);margin-top:6px;">${t('successMsg', ROOM.players[ROOM.finishedBy].name)}</p>`:'')}
    <div style="margin-top:18px; padding-top:14px; border-top:1px solid var(--line);">
      <div style="font-size:12px;letter-spacing:2px;text-transform:uppercase;color:var(--muted);margin-bottom:8px;">${t('runningTotals')}</div>
      ${ROOM.turnOrder.map(uid=>`<div style="display:flex;justify-content:space-between;padding:3px 0;">
        <span>${ROOM.players[uid].name} ${ROOM.players[uid].eliminated?` <span style="color:var(--crimson)">(${t('eliminatedTag')})</span>`:''}</span><b style="color:var(--brass-soft)">${ROOM.players[uid].total}</b>
      </div>`).join('')}
    </div>
    ${ROOM.hostUid===myUid
      ? `<button class="primary-btn" style="margin-top:18px" data-action="actNextRound">${ROOM.turnOrder.filter(u=>!ROOM.players[u].eliminated).length<=1? t('seeResultBtn'):t('nextRoundBtn')}</button>`
      : `<div class="small-note" style="text-align:center;margin-top:18px">${t('waitingForHost')}</div>`}
  </div>
  <div class="ad-slot" id="adSlotReveal">
    <span class="ad-slot-label">Ad space</span>
  </div>`;
}
...
function renderGameOver(){
  ...
  <div class="setup-card" style="text-align:center">
    ...
    <div style="font-family:Georgia,serif;font-size:26px;color:var(--brass-soft);margin:14px 0;">${t('winsLabel', winner.name)}</div>
    ...
    <button class="primary-btn" style="margin-top:18px" data-action="actLeaveToMenu">${t('playAgainBtn')}</button>
  </div>`;
}
...
function render(){
  ...
  let html = '';
  if(!(ROOM && ROOM.phase==='playing' && ROOM.currentUid===myUid && !ROOM.drawnCard && !ROOM.modal)){ burnDeclared = false; }
  ...
  html = screenHeader(`${t('round')} ${ROOM.round}${ROOM_CODE? ' · '+ROOM_CODE : ''}`, true);
  ...
  html += `<div class="human-block ${myActiveNow?'active-turn':''}" style="${colorStyleVars(myCol)}">
    ...
    <div class="player-row-top">
      ...
      <div class="player-namecol">
        <div class="player-name"><span class="nm">${t('you')}</span></div>
        <span class="badge total">${me.total} ${t('pts')}</span>
      </div>
    </div>
    ${me.eliminated
      ? `<div style="text-align:center;color:var(--muted);font-size:13px;padding:14px 0;">${t('spectating')}</div>`
...
function attach(){
  ...
  app.onclick = (e)=>{
    ...
    switch(action){
      ...
      case 'actDeclareFinished': actDeclareFinished(); break;
